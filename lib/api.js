...
function parsePost({ fields }) {
  return {
    ...
    category: fields.category ? fields.category.map(cat => cat.fields.slug) : null,
  }
}
...
function parseCategory({ fields }) {
  return {
    name: fields.name,
    slug: fields.slug,
    posts: fields.posts ? fields.posts.map(post => post.fields.slug) : null,
  }
}
...
export async function getAllCategoriesForHome() {
  ...
  return parseCategoryEntries(entries)
}
